<?php
declare(strict_types=1);namespace Medas\ServiceManager;use function A\B;use function A\C;use function A\D;use function B\E;use const C;#[Attributes\Service(12)]abstract class ServiceInstantiator{use Cheese;private const CONST_NAME=[1,2,3,4,5];public?string$cheese=null;public function callback(int&$a,?array$b=[],bool&...$questions):?callable{self::$variable=10;return fn($a)=>strlen($a);}public function ternary():bool{$i=$a?true:false;$j=$a?:false;}protected function test(RelativePath\RelativeClass$relativeClass,bool$isDefault=false,callable...$callableArray){printf("\e[%sm%s\e[0m",implode(';',$formats),$string);for($i=0;$i<10;++$i){// Test
